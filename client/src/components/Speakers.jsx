import React from "react";

const SpeakerCard = ({ image, name, title, company }) => (
  <div className="relative p-4 bg-white shadow-lg rounded-lg flex flex-col items-center transition-transform transform hover:scale-105 hover:shadow-xl hover:bg-gray-50">
    <div className="absolute inset-0 border-b-4 border-transparent hover:border-blue-500 transition-all duration-300"></div>
    <img src={image} alt={name} className="w-24 h-24 rounded-full mb-4" />
    <h3 className="font-bold text-lg relative z-10 text-center">{name}</h3>
    <p className="text-sm text-gray-700 relative z-10 text-center">{title}</p>
    <p className="text-sm text-gray-500 relative z-10 text-center">{company}</p>
  </div>
);

const SpeakerSection = ({ title, speakers }) => (
  <div className="w-full md:w-full px-4">
    <h2 className="text-xl font-bold text-center mb-4">{title}</h2>
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
      {speakers.map((speaker, index) => (
        <SpeakerCard
          key={index}
          image={speaker.image}
          name={speaker.name}
          title={speaker.title}
          company={speaker.company}
        />
      ))}
    </div>
  </div>
);

const imageLink =
  "data:image/jpeg;base64,"

const EventSpeakers = () => {
  const chiefGuest = {
    image: "https://www.iitg.ac.in/sukumar/assets/img/sukumar.png",
      name: "Professor Sukumar Nandi",
      title: "Senior Professor",
      company: "Indian Institute of Technology Guwahati",
  };

  const academicSpeakers = [
    {
      image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcSbe90WdUkdGiYYxP9C-tqNSGw_axvE6d5e3g&s",
      name: "Dr. Mohammad Aazam",
      title: "Professor",
      company: "Carnegie Mellon University",
    },
    {
      image: "https://cde.nus.edu.sg/ece/wp-content/uploads/sites/3/2019/03/Mohan-Gurusamy-PPhoto-removebg-preview-e1704788720754.jpg",
      name: "Dr. Mohan Gurusamy",
      title: "Professor",
      company: "National University of Singapore",
    },
    {
      image: "https://i1.rgstatic.net/ii/profile.image/1067960168878081-1631632954277_Q512/Alessandro-Brighente.jpg",
      name: "Dr Alessandro Brighente",
      title: "Professor",
      company: "University of Padova",
    },
    {
      image: "https://i1.rgstatic.net/ii/profile.image/956966646083589-1605170036592_Q512/Bharatesh-Chakravarthi.jpg",
      name: "Dr Bharatesh C",
      title: "Professor",
      company: "Arizona State University",
    },
    {
      image: "https://d502jbuhuh9wk.cloudfront.net/orgData/6644941cd8873d3d937118c0/pages/assets/images/arXA81675976220704.jpg",
      name: "Dr Pranav Nerurkar",
      title: "Specialist-AI, Data Science",
      company: "Utkarsh Minds",
    },
    {
      image: "https://media.licdn.com/dms/image/v2/C5603AQGmi7b6j1ULKw/profile-displayphoto-shrink_800_800/profile-displayphoto-shrink_800_800/0/1628573086957?e=1729728000&v=beta&t=hJ5yfiIn10HbXMdAky1UERCd7uPKeXRHpBoPmsP7-Zw",
      name: "Dr Palash Ingle",
      title: "Research Professor",
      company: "Sejong University",
    },
    {
      image: "https://alakesh-kalita.github.io/assets/images/profile.jpg",
      name: "Dr Alakesh Kalita",
      title: "Assistant Professor",
      company: "Indian Institute of Technology (ISM) Dhanbad",
    },
    {
      image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcR5Yc8bLpvqpKg9h3XxV_aSIkv7cSzheAUPNw&s",
      name: "Dr. Saraju P. Mohanty",
      title: "Professor",
      company: "University of North Texas",
    },
    {
      image: "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTQIkGxn1TC531xACVKcnEpJ2GzA1EPT-x6OQ&s",
      name: "Dr. Sajal K. Das",
      title: "Curators' Distinguished Professor",
      company: "Missouri University of Science and Technology.",
    }
  ];

  const industrySpeakers = [
    {
      image: "https://scholar.googleusercontent.com/citations?view_op=medium_photo&user=uOLQy8MAAAAJ&citpid=2",
      name: "Dr. Vishal J. Rathod",
      title: "Senior Project Engineer",
      company: "CDAC Bangalore",
    }
  ];

  return (
    <div className="container mx-auto px-4 py-8">
    <div className="mb-8">
      <h2 className="text-2xl font-bold text-center mb-4">Chief Guest</h2>
      <div className="flex justify-center">
        <SpeakerCard {...chiefGuest} />
      </div>
    </div>

    <div className="flex flex-col gap-8" id="speakers">
      <SpeakerSection title="Academic Speakers" speakers={academicSpeakers} />
      <SpeakerSection title="Industry Speakers" speakers={industrySpeakers} />
    </div>
  </div>
  );
};

export default EventSpeakers;